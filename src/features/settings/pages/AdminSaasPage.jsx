...
function getPlanLabel(plan) {
  if (!plan) return '-';
  return plan.nom || plan.code_plan || plan.code || '-';
}
...
export default function AdminSaasPage() {
  ...
  async function loadData() {
    try {
      ...
      if ((plansData || []).length > 0) {
        setForm((prev) => ({
          ...prev,
          plan_id: prev.plan_id || plansData[0].id,
        }));
      }
    } catch (err) {
  ...
  const selectedPlan = useMemo(() => {
    return plans.find((item) => item.id === form.plan_id) || null;
  }, [plans, form.plan_id]);
        ...
        <form className="sales-form-grid" onSubmit={handleCreateEntreprise}>
          ...
          <div className="form-group">
            ...
            <input
              ...
              value={
                selectedPlan
                  ? ['pro', 'premium'].includes(getPlanCode(selectedPlan))
                    ? 'Oui'
                    : 'Non'
                  : '-'
              }
          ...
          <div className="form-group">
            ...
            <input
              type="text"
              value={
                selectedPlan?.max_utilisateurs ??
                (getPlanCode(selectedPlan) === 'premium'
                  ? 50
                  : getPlanCode(selectedPlan) === 'pro'
                  ? 10
                  : getPlanCode(selectedPlan) === 'basic'
                  ? 3
                  : '-')
              }
              readOnly
                ...
                {entreprises.map((item) => {
                  ...
                  return (
                    <tr key={item.id}>
                      ...
                      <td>{item.plan_nom || item.code_plan || item.plan_abonnement || '-'}</td>
                      ...
                      <td>
                        {item.nombre_membres ?? 0} / {item.utilisateurs_max ?? item.max_utilisateurs ?? '-'}
                      </td>

                      <td>
                        {(item.ia_active ?? (item.ia_niveau && item.ia_niveau !== 'none')) ? 'Oui' : 'Non'}
                      </td>
